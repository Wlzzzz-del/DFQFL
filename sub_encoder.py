...
class MyEncoder(Encoder, metaclass=ABCMeta):
    ...
    @tq.static_support
    def forward(self, qdev: tq.QuantumDevice, x, _adding):
        for info in self.func_list:# 这边是一个一个循环
            ...
            func_name_dict[info["func"]](
                qdev,
                wires=[info["wires"][0]+_adding],# 这个wires只能放置一个qubit，一会看怎么循环放进来
                params=params,
